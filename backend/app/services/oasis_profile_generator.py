...
class OasisProfileGenerator:
    ...
    @zep_client.setter
    def zep_client(self, value) -> None:
        # ponytail: object.__new__ tests assign .zep_client; wrap as ZepCloudBackend
        if value is None:
            return
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        try:
            ...
            all_facts = set(edge_result.facts or [])
            for edge in edge_result.edges or []:
                fact = edge.get("fact") if isinstance(edge, dict) else getattr(edge, "fact", None)
                if fact:
                    all_facts.add(fact)
            results["facts"] = list(all_facts)

            all_summaries = set()
            for node in node_result.nodes or []:
